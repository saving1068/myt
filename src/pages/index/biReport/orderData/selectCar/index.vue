    ...
    <div class="test-drive boxshadow">        
        ...
        <div class="test-drive-filter">
            <div class="test-drive-filter__item">
                ...
                <el-select v-model="queryParams.querySaleMode" size="mini" placeholder="预订方式" clearable @change="initData" class="selects">
                    <el-option
                        v-for="item in salemodeList"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value">
                    </el-option>
                ...
                <el-select v-model="queryParams.queryStoreId" size="mini" placeholder="全部销售分组" style='width: 130px;' clearable @change="getStore" class="selects">
                    <el-option
        ...
        <panel class="data-box">
            ...
            <div class="test-drive__content">
                <box class="table-box">
                    ...
                    <el-table :data="tableData" style="width:100%;" class="tableList" fit v-loading="tableLoading">
                        ...
                        <el-table-column prop="adviserName" label="接单顾问" width="100" align="center" />
                        ...
                        <el-table-column label="车系车型" min-width="200" align="center">
                            <template slot-scope="scope">
                                {{scope.row.brandName}}
                        ...
                        <el-table-column label="退车时间" width="100" align="center">
                            <template slot-scope="scope">
                                <span v-if="scope.row.backDay">
                                    {{scope.row.backDay}}<br>
                                    {{scope.row.backHour}}
                                </span>
                                <span v-else>--</span>
                            </template>
                        </el-table-column>
                        <el-table-column label="成交时间" width="100" align="center">
                            <template slot-scope="scope">
                                <span v-if="scope.row.dealDay">
                                    {{scope.row.dealDay}}<br>
                                    {{scope.row.dealHour}}
                                </span>
                                <span v-else>--</span>
                            </template>
                        </el-table-column>
                        <el-table-column label="订金金额" align="center">
                            <template slot-scope="scope">
                                <span class="price" v-if="scope.row.earnestMoney">￥{{scope.row.earnestMoney}}</span>
                                <span v-else>--</span>
                            </template>
                        </el-table-column>
                        <el-table-column prop="ordersId" label="预订方式" align="center">
                            <template slot-scope="scope">
                                {{scope.row.saleMode == 2 ? '领券下单' : '选车下单'}}
                                <span v-if="scope.row.saleMode == 2 && scope.row.reserveCarDesc">({{scope.row.reserveCarDesc}})</span>
                            </template>
                        </el-table-column>
...
import {reservecarList, getOrderStatistics} from '@/api/bi'
...
export default {
    ...
    async created() {
        ...
        try {
            const [brands, stores, statistics] = await Promise.all([
                getSaleBrandDtos(),
                getStoreList(),
                getOrderStatistics({orderType: 2})
            ])
    ...
    methods: {
        async getData(e) {
            ...
            try {
                const res = await reservecarList(this.queryParams)
        ...
        async exportExcel() {
            ...
            try {
                const res = await reservecarList({loadMode: 1, ...this.queryParams})
                const list = res.items.map(v => {
                    v.saleModeType = v.saleMode == 1 ? '选车下单' : '领券下单'
                 ...
                 if (list.length) {
                    const params = {
                        title: ['客户姓名', '客户手机', '意向', '客户来源', '接单顾问', '顾问手机', '接单门店', '订单号', '车系车型', '下单时间', '退车时间', '成交时间', '订金金额', '营销方式', '订单状态', '下单方式'],
                        key: ['realName', 'mobile', 'levelId', 'source', 'adviserName', 'adviserMobile', 'storeName', 'ordersId', 'brandName,carTypeName', 'placeOrderDay,placeOrderHour', 'backDay,backHour', 'dealDay,dealHour', 'earnestMoney', 'saleModeType', 'orderStatusDesc', 'orderType'],
                        data: list,
                        autoWidth: true,
                        filename: '选车单报表'
                    }
